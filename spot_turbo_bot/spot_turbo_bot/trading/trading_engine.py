...
class TradingEngine:
    def __init__(self, http: BinanceHTTP, exinfo: ExchangeInfo, state: GlobalState, audit=None):
        ...
        self._trade_seq = 0
        self._buy_logged: Dict[str, bool] = {}
        self._sell_logged: Dict[str, bool] = {}
        self._last_sell_order_id: Dict[str, str] = {}
    ...
    def _ensure_position_log_key(self, position: Position) -> str:
        if position.log_key:
            key = position.log_key
        else:
            self._trade_seq += 1
            key = f"{position.slot}:{position.symbol.upper()}:{position.opened_at_ts}:{self._trade_seq}"
            position.log_key = key
        self._buy_logged.setdefault(key, False)
        self._sell_logged.setdefault(key, False)
        return key

    def _format_hold(self, seconds: int) -> str:
        seconds = max(0, int(seconds))
        h, rem = divmod(seconds, 3600)
        m, s = divmod(rem, 60)
        return f"{h:02d}:{m:02d}:{s:02d}"

    def _maybe_log_buy(self, position: Position) -> None:
        key = self._ensure_position_log_key(position)
        if position.qty_base <= 0.0:
            return
        if self._buy_logged.get(key):
            return
        ts = now_ksa().isoformat(timespec="seconds")
        log_line(
            f"[BUY] {position.symbol.upper()} entry={position.entry_price:.8f} "
            f"qty={position.qty_base:.8f} quote_in={position.entry_quote_spent:.2f} "
            f"slot={position.slot} t={ts}"
        )
        self._buy_logged[key] = True
        self._clear_entry_skip(position.symbol)

    def _emit_trade_logs(self, trade: Optional[dict[str, Any]]) -> None:
        if not trade:
            return
        key = trade.get("log_key")
        if not key or self._sell_logged.get(key):
            return
        symbol = trade["symbol"]
        qty = trade["qty"]
        quote_out = trade["quote_out"]
        pnl_pct = trade["pnl_pct"] * 100.0
        hold_str = trade["hold_str"]
        exit_avg = trade["exit_avg"]
        t_close = trade["t_close"].isoformat(timespec="seconds")
        prefix = trade["log_prefix"]
        if prefix == "SELL-TP":
            log_line(
                f"[SELL-TP] {symbol} exit={exit_avg:.8f} qty={qty:.8f} "
                f"quote_out={quote_out:.2f} pnl%={pnl_pct:+.2f} held={hold_str} t={t_close}"
            )
        else:
            reason_display = trade["reason_display"]
            log_line(
                f"[SELL-MKT] {symbol} exit={exit_avg:.8f} reason={reason_display} "
                f"qty={qty:.8f} quote_out={quote_out:.2f} pnl%={pnl_pct:+.2f} held={hold_str} t={t_close}"
            )
        self._sell_logged[key] = True

    def _record_trade_report(self, trade: Optional[dict[str, Any]]) -> None:

    def _determine_exit_method(self, reason_key: Optional[str], reason_label: str) -> tuple[str, str, str]:
        rk = (reason_key or "").lower()
        label = (reason_label or "").upper()
        if label == "TP" or rk in {"take_profit", "tp"}:
            return "TP_LIMIT_MAKER", "SELL-TP", ""
        if label == "STOP" or rk == "stop":
            return "MARKET_STOP", "SELL-MKT", "STOP"
        if label == "PULLBACK" or rk == "pullback":
            return "MARKET_PULLBACK", "SELL-MKT", "PULLBACK"
        return "MARKET_UNWIND", "SELL-MKT", "UNWIND"

    def _estimate_sell_fee(self, commission: float, commission_asset: Optional[str], exit_avg: float, proceeds_quote: float, symbol: str) -> float:
        if commission <= 0.0:
            return proceeds_quote * FEE_SELL_RATE
        asset = (commission_asset or "").upper()
        if asset == "USDT":
            return commission
        base_asset = symbol[:-4] if symbol.endswith("USDT") else ""
        if asset == base_asset and exit_avg > 0.0:
            return commission * exit_avg
        return proceeds_quote * FEE_SELL_RATE

        if not trade:
            return
        report = getattr(market_ctx, "report", None)
        if report is None or not hasattr(report, "trade"):
            return
        try:
            report.trade(
                symbol=trade["symbol"],
                ...
                exit_px=trade["exit_avg"],
                quote_in=trade["quote_in"],
                quote_out=trade["quote_out"],
                fees_buy=trade["fees_buy"],
                fees_sell=trade["fees_sell"],
                pnl_abs=trade["pnl_abs"],
                pnl_pct=trade["pnl_pct"],
                held_seconds=trade["held_secs"],
                buy_order_id=trade["buy_order_id"],
                sell_order_id=trade["sell_order_id"],
                t_open=trade["t_open"],
                t_close=trade["t_close"],
            )
        except Exception:
            pass
    ...
    async def _handle_buy_signal_rewired(self, req: BuyRequest) -> Optional[Position]:
        ...
        async with self._buy_lock:
            ...
            entry_fee_quote = 0.0
            base_asset = s[:-4] if s.endswith("USDT") else ""
            for fill in fills:
                try:
                ...
                if fee <= 0.0:
                    continue
                asset = (fill.get("commissionAsset") or "").upper()
                if asset == "USDT":
                    entry_fee_quote += fee
                elif asset == base_asset:
                    try:
                        fill_price = float(fill.get("price", worst_price) or worst_price)
                    except Exception:
                        fill_price = worst_price
                    entry_fee_quote += fee * fill_price
            if entry_fee_quote <= 0.0:
                entry_fee_quote = cumm_quote * FEE_BUY_RATE
            ...
            pos = Position(
                ...
                opened_at_ts=int(now_ksa().timestamp()),
                slot=slot.name,
                open_1m=req.open_1m,
            ...
            slot.position = pos
            pos.buy_order_id = order_id or client_id_ack or None
            pos.entry_fee_quote = entry_fee_quote
            self._ensure_position_log_key(pos)
            self._maybe_log_buy(pos)
            ...
            log_buy(
                ...
                entry=avg_price,
                open_1m=req.open_1m,
                target_p5=req.target_p5,
                tp_p10=req.tp_p10,
                sl_m5=req.sl_m5,
                quote_spent=cumm_quote,
            )

            # Daily text report for BUY (best effort, does not affect logic)
            try:
                if getattr(market_ctx, 'report', None) is not None:
                    try:
                        target_pct_val = ((float(req.target_p5) / float(avg_price)) - 1.0) if (req.target_p5 and avg_price) else 0.0
                    except Exception:
                        target_pct_val = 0.0
                    market_ctx.report.buy(
                        symbol=s,
                        slot=slot.name,
                        entry=float(avg_price),
                        open_1m=float(req.open_1m or avg_price),
                        target_pct=float(target_pct_val*100.0),
                        target_price=float(req.target_p5 or avg_price),
                        tp_pct=float(_EXIT_V2_TAKE_LIMIT_PCT*100.0),
                        tp_price=float(req.tp_p10 or take_limit_px),
                        sl_pct=float(_EXIT_V2_STOP_DROP_PCT*100.0),
                        sl_price=float(req.sl_m5 or stop_px),
                        quote_spent=float(cumm_quote),
                        allowed_quote=float(cumm_quote),
                        planned_quote=float(cumm_quote),
                        coverage_pct=1.0,
                        exp_slip_pct=None,
                        real_slip_pct=None,
                    )
    ...
    def on_sell_closed(
        ...
        *,
        ...
        cumm_quote: Optional[float] = None,
    ) -> None:
        ...
        pos = slot.position
        symbol_u = symbol.upper()
        controller = self._exit_controllers.get(symbol_u)

        qty = executed_qty if executed_qty > 0.0 else pos.qty_base
        if qty <= 0.0:
            qty = pos.qty_base
        qty = max(0.0, qty)
        exit_avg = exit_price if qty <= 0.0 else (proceeds_quote / qty if qty > 0.0 else exit_price)

        if event_time_ms and event_time_ms > 0:
            t_close_dt = datetime.fromtimestamp(event_time_ms / 1000.0, tz=datetime.timezone.utc).astimezone(KSA_TZ)
        else:
            t_close_dt = now_ksa()
        t_open_dt = datetime.fromtimestamp(pos.opened_at_ts, tz=KSA_TZ)
        held_secs = int((t_close_dt - t_open_dt).total_seconds())

        reason_key = (reason or "").lower()
        if not reason_key:
            reason_key = self._exit_close_reason.get(symbol_u, "filled")
        reason_label = {
            "take_profit": "TP",
            "stop": "STOP",
            "pullback": "PULLBACK",
            "market_exit": "MARKET",
        }.get(reason_key, reason.upper() if reason else "FILLED")

        method, log_prefix, reason_display = self._determine_exit_method(reason_key, reason_label)

        sell_order_id = order_id or self._last_sell_order_id.get(symbol_u) or client_order_id or pos.sell_order_id
        fees_buy = pos.entry_fee_quote if pos.entry_fee_quote > 0.0 else pos.entry_quote_spent * FEE_BUY_RATE
        fees_sell = self._estimate_sell_fee(commission, commission_asset, exit_avg, proceeds_quote, symbol_u)
        pos.exit_fee_quote = fees_sell
        if sell_order_id:
            pos.sell_order_id = sell_order_id

        pnl_abs = proceeds_quote - pos.entry_quote_spent
        pnl_pct = (exit_avg / pos.entry_price - 1.0) if pos.entry_price > 0 else 0.0

        log_key = self._ensure_position_log_key(pos)
        trade = {
            "log_key": log_key,
            "symbol": symbol_u,
            "slot": slot.name,
            ...
            "quote_out": proceeds_quote,
            "fees_buy": fees_buy,
            "fees_sell": fees_sell,
            "pnl_abs": pnl_abs,
            "pnl_pct": pnl_pct,
            "held_secs": held_secs,
            "hold_str": self._format_hold(held_secs),
        ...
        slot.balance_quote += proceeds_quote
        self._last_spot_free += proceeds_quote
        slot.position = None
