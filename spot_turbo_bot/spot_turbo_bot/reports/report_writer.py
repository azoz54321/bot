...
f"exit={exit_price:.8f} entry={entry_price:.8f} pnl_pct={(pnl_pct*100):+.2f}% pnl_usdt={pnl_usdt:+.2f} hold={_hms(hold_seconds)}",
    ...
    def trade(
        ...
        *,
        ...
        try:
            open_iso = t_open.astimezone(KSA).replace(microsecond=0).isoformat()
        except Exception:
            open_iso = str(t_open)
        try:
            close_iso = t_close.astimezone(KSA).replace(microsecond=0).isoformat()
        except Exception:
            close_iso = str(t_close)
        self._append_line(
            f"[TRADE] {symbol} side={side} method={method} qty={qty_base:.8f} "
            f"entry={entry_px:.8f} exit={exit_px:.8f} quote_in={quote_in:.2f} "
            f"quote_out={quote_out:.2f} fees_buy={fees_buy:.4f} fees_sell={fees_sell:.4f} "
            f"pnl_abs={pnl_abs:+.2f} pnl_pct={pnl_pct:+.2f}% held={_hms(held_seconds)} "
            f"t_open={open_iso} t_close={close_iso} "
            f"buy_order_id={buy_order_id or '-'} sell_order_id={sell_order_id or '-'}",
            ts=t_close,
        )
